...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/skills?${strapiParams}`, {
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch skills' }, { status: response.status })
    }
