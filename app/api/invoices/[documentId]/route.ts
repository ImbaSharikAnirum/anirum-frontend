...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const url = `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/invoices/${documentId}${queryString ? `?${queryString}` : ''}`
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/invoices/${documentId}`, {
      method: 'PUT',
      headers: {
