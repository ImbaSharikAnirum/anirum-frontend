...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL}/invoices/${documentId}${queryString ? `?${queryString}` : ''}`
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/invoices/${documentId}`, {
      method: 'PUT',
      headers: {
